...
def save_new_order(product, quantity, total_price, location):
    ...
    send_telegram_message(
           ...
           f"expires_at: {(datetime.now() + timedelta(hours=4)).isoformat()}"
    )
...
Location and Extra details: {{ order_info['location'] | replace('\n', ' ') }}`;
            new Notification("New Order Received!", { body: details });
...
<div class="container">
    <h1>Moo Chapatis</h1>
    <div class="slogans">
    <div>Order, we Deliver</div>
    <div>All Deliveries from 7.pm - 10.pm</div>
</div>
